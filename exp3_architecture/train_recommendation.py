...
class TrainConfig:
    ...
    # Feature enrichment: append book_train_top30_l1 to music_top30_l1 (30→60 dims)
    use_enriched_features: bool = False
    top_book_genres: int = 30
    # Rating-filtered BPR: only use ≥threshold-rated train interactions as positives
...
def build_enriched_user_features(
    ...
    """Return [music_top30_l1 | book_train_top{k}_l1] as (N, 30+k) float32 array.

    The book genre profile is loaded from the Stage-1 leakage-safe artifact
    (built from target-train interactions only).  Columns are ordered by global
    book genre frequency (book_genre_index.csv), so [:top_book_genres] selects
    the most frequent categories without any additional lookup.
    """
...
def build_highrated_positive_cache(
    ...
    """Build a BPR positive cache using only ≥threshold-rated train interactions.

    Graph construction and evaluation always use the full train_matrix so the
    experimental protocol is unchanged; only the BPR sampling signal is filtered.
    """
...
def train_one_model(
    ...
    # --- User features (optionally enriched) ---
    ...
    in_dim = user_features_np.shape[1]  # picks up enriched dim (e.g. 60) automatically
    ...
    # Negative sampling distribution always built from full train popularity
    ...
    # BPR positive cache: full train or rating-filtered
    ...
    for epoch in range(1, train_config.max_epochs + 1):
        ...
        # --- Phase 3: Validation evaluation (GPU) ---
    ...
    # Per-user alpha stats (if applicable)
    alpha_stats = {}
...
def instantiate_model(
    ...
    elif model_spec.model_kind == "chebnet":
        fused_operator = (
            model_spec.alpha_value * source_operator
            + (1.0 - model_spec.alpha_value) * target_operator
        ).coalesce()

        return ChebNetRecommender(
    ...
    elif model_spec.model_kind == "graphsage":
        fused_operator = (
            model_spec.alpha_value * source_operator
            + (1.0 - model_spec.alpha_value) * target_operator
        ).coalesce()
    ...
    elif model_spec.model_kind == "gat":
        fused_operator = (
            model_spec.alpha_value * source_operator
            + (1.0 - model_spec.alpha_value) * target_operator
        ).coalesce()

        return GATRecommender(
            operator=fused_operator,
            in_dim=x_user_dim,
            n_items=n_items,
            genre_dim=genre_dim,
